...
`wontfix` already existed in the repo as a GitHub default label. The other four were created
when this configuration was written, so every role in the table resolves to a label that
actually exists — applying one will not fail.
